...
public class SchedulingController {
    ...
    public ResponseEntity<Void> deleteSlot(@PathVariable("slotId") Long slotId) {
        schedulingService.deleteSlot(slotId);
        return ResponseEntity.ok().build();
    }

    @PutMapping("/slots/{slotId}/notes")
    @PreAuthorize("hasAnyRole('ADMIN', 'TRAINER')")
    public ResponseEntity<SlotResponse> updateNotes(@PathVariable("slotId") Long slotId, @RequestBody String notes) {
        return ResponseEntity.ok(schedulingService.updateNotes(slotId, notes));
    }
}
