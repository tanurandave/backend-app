...
public class CourseService {
        ...
        private CourseResponse mapToResponse(Course course) {
                ...
                return CourseResponse.builder()
                                ...
                                .hasSyllabus(course.getSyllabusData() != null && course.getSyllabusData().length > 0)
        ...
        @Transactional
        public CourseResponse uploadSyllabus(Long courseId, MultipartFile file) {
                Course course = courseRepository.findById(courseId)
                                .orElseThrow(() -> new RuntimeException("Course not found"));

                try {
                        course.setSyllabusData(file.getBytes());
                        course.setSyllabusFileName(file.getOriginalFilename());
                        course.setSyllabusContentType(file.getContentType());
                        courseRepository.save(course);
                } catch (IOException e) {
                        throw new RuntimeException("Failed to store syllabus file", e);
                }
        ...
        @Transactional
        public CourseResponse deleteSyllabus(Long courseId) {
                Course course = courseRepository.findById(courseId)
                                .orElseThrow(() -> new RuntimeException("Course not found"));

                course.setSyllabusData(null);
                course.setSyllabusFileName(null);
                course.setSyllabusContentType(null);
                courseRepository.save(course);

                return mapToResponse(course);
        }

        public Course getSyllabusData(Long courseId) {
                Course course = courseRepository.findById(courseId)
                                .orElseThrow(() -> new RuntimeException("Course not found"));
                if (course.getSyllabusData() == null) {
                        throw new RuntimeException("No syllabus uploaded for this course");
                }
                return course;
        }

        public List<CourseResponse> getCoursesByTrainerId(Long trainerId) {
                return courseRepository.findByPrimaryTrainerId(trainerId).stream()
                                .map(this::mapToResponse)
                                .collect(Collectors.toList());
        }
}
